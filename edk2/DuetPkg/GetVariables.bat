...
@for /f "tokens=1*" %%i in ('type %CONFIG_FILE% ^| find "ACTIVE_PLATFORM" ^| find /V "#" ^| find /V "TARGET_ARCH"') do @set DSC_FILE%%j
@set DSC_FILE=%DSC_FILE:/=\%

@for /f "tokens=1*" %%i in ('type %DSC_FILE% ^| find "OUTPUT_DIRECTORY" ^| find /V "#"') do @set OUT_DIR%%j
@set OUT_DIR=%OUT_DIR:/=\%
@set OUT_DIR=%OUT_DIR: =%
